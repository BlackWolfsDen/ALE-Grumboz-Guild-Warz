...
CREATE TABLE IF NOT EXISTS `commands` (
  ...
  `GM_minimum` varchar(6) NOT NULL DEFAULT 'cghi' COMMENT 'Minimum Required Gm security level to use GM level commands.',
  ...
  `pig_L` smallint(5) NOT NULL DEFAULT '10' COMMENT 'how many pigs per location.',
  ...
  `anarchy` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT 'UN-SUPPORTED pure GvG PvP no faction filter.',
  `f_timer` tinyint(20) unsigned NOT NULL DEFAULT '0' COMMENT 'applies a cooldown-timer when a flag spaws. 0=off::1=on',
  `s_timer` bigint(20) unsigned NOT NULL DEFAULT '300000' COMMENT 'no-tag-flag timer cooldown time in seconds. i.e. 5 minutes = 300000 seconds.',
  `guild_id` bigint(20) unsigned NOT NULL DEFAULT '0' COMMENT 'Guild ID',
  `guild_invite` bigint(1) unsigned NOT NULL DEFAULT '0' COMMENT 'UN-SUPPORTED guild invite system. 0 == off :: 1 == on',
  PRIMARY KEY (`guild`),
  UNIQUE KEY `guild` (`guild`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table guild_warz_335a.commands: ~1 rows (approximately)
...
REPLACE INTO `commands` (`guild`, `commands`, `info_loc`, `list_loc`, `tele`, `version`, `loc`, `farm`, `barrack`, `hall`, `pig`, `guard`, `GLD_lvlb`, `GLD_lvls`, `respawn_flag`, `details_loc`, `table`, `GM_admin`, `GM_minimum`, `currency`, `loc_cost`, `farm_cost`, `barrack_cost`, `hall_cost`, `pig_cost`, `guard_cost`, `farm_L`, `barrack_L`, `hall_L`, `pig_L`, `guard_L`, `pig_payz`, `gift_count`, `flag_require`, `Server`, `flag_id`, `farm_id`, `barrack_id`, `hall_id`, `pig_id`, `guard_id`, `x1`, `x2`, `x3`, `command_set`, `anarchy`, `f_timer`, `s_timer`, `guild_id`, `guild_invite`) VALUES
	('SERVER', 'commands', 'info', 'list', 'gtele', 'ver', 'area', 'farm', 'barrack', 'hall', 'pig', 'guard', 0, 0, 'flag', 'loc', 'table', 'az', 'cghi', 62006, 10, 5, 10, 100, 1, 1, 1, 1, 1, 20, 10, 100000, 25, 0, 'SERVER', 187432, 500000, 500002, 500004, 49000, 49002, 0, 0, 0, 'set', 0, 1, 300000, 0, 0);
/*!40000 ALTER TABLE `commands` ENABLE KEYS */;


-- Dumping structure for table guild_warz_335a.help
...
-- Dumping data for table guild_warz_335a.help: ~60 rows (approximately)
...
REPLACE INTO `help` (`entry`, `name`, `description`, `example`, `command_level`) VALUES
	...
	(38, 'pig_L', 'system setting limit how may pigs per farm.', 'set pig_L 20', '6'),
	(39, 'guard_L', 'system setting limit how many guards per barrack.', 'set guard_L 10', '6'),
	...
	(56, 'anarchy', 'game core setting allows same team invasions . 0 = off::1 = on', 'LOCKED - NOT SUPPORTED .  set anarchy 1', '7'),
	...
	(60, 'guild_invite', 'Guild Level Setting. Guild invite system 0 == off :: 1 == on // default on.', 'LOCKED - NOT SUPPORTED . set guild_invite 1', '7');
/*!40000 ALTER TABLE `help` ENABLE KEYS */;


-- Dumping structure for table guild_warz_335a.zones
CREATE TABLE IF NOT EXISTS `zones` (
  ...
  `fs_time` bigint(20) NOT NULL DEFAULT '0',
  `guild_id` bigint(20) NOT NULL DEFAULT '0',
  PRIMARY KEY (`entry`),
  UNIQUE KEY `entry` (`entry`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table guild_warz_335a.zones: ~1 rows (approximately)
